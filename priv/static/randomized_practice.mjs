...
function random_int(seed, from2, to) {
  const range = to - from2 + 1;
...
function to_iterator(generator, seed) {
  return unfold(
    seed,
    (seed2) => {
      let $ = step(generator, seed2);
      let value3 = $[0];
      let new_seed2 = $[1];
      return new Next(value3, new_seed2);
    }
  );
}
function to_random_iterator(generator) {
  return to_iterator(generator, random2());
}
function random_sample(generator) {
  let $ = first2(to_random_iterator(generator));
  if (!$.isOk()) {
    throw makeError(
      "assignment_no_match",
      "prng/random",
...
function int3(from2, to) {
  return new Generator(
    (seed) => {
      let $ = sort_ascending(from2, to, compare);
      let low = $[0];
      let high = $[1];
...
// build/dev/javascript/randomized_practice/randomized_practice.mjs
var Cue = class extends CustomType {
...
var WelcomeView = class extends CustomType {
};
var InputView = class extends CustomType {
};
var OutputView = class extends CustomType {
};
var Model = class extends CustomType {
  constructor(view2, number_of_shots_form, number_of_shots_to_go, club_form, club_stored) {
    super();
    this.view = view2;
    this.number_of_shots_form = number_of_shots_form;
    this.number_of_shots_to_go = number_of_shots_to_go;
    this.club_form = club_form;
    this.club_stored = club_stored;
  }
};
var UserClickButton = class extends CustomType {
};
var UserChangeFirstClubName = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangeSecondClubName = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangeThirdClubName = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangeNumberOfShots = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserGoesToNextShot = class extends CustomType {
};
var UserGoesToInput = class extends CustomType {
};
var ClubNames = class extends CustomType {
  constructor(first5, second, third) {
    super();
    this.first = first5;
    this.second = second;
    this.third = third;
...
function init2(_) {
  return new Model(
    ...
    "24",
    0,
    new ClubNames("I9", "I7", "I5"),
    new ClubNames("", "", "")
  );
}
function handle_shots_input(text3) {
  let $ = parse(text3);
  if ($.isOk()) {
    let value3 = $[0];
    return value3;
...
function handle_next_shot(model) {
  let $ = model.number_of_shots_to_go > 0;
  if (!$) {
    return model.withFields({ view: new InputView() });
  } else {
    return model.withFields({
      number_of_shots_to_go: model.number_of_shots_to_go - 1
    });
  }
}
function update(model, msg) {
  if (msg instanceof UserChangeFirstClubName) {
    let text3 = msg[0];
    return model.withFields({
      club_form: model.club_form.withFields({ first: text3 })
    });
  } else if (msg instanceof UserChangeSecondClubName) {
    let text3 = msg[0];
    return model.withFields({
      club_form: model.club_form.withFields({ second: text3 })
    });
  } else if (msg instanceof UserChangeThirdClubName) {
    let text3 = msg[0];
    return model.withFields({
      club_form: model.club_form.withFields({ third: text3 })
    });
  } else if (msg instanceof UserChangeNumberOfShots) {
    let text3 = msg[0];
    return model.withFields({ number_of_shots_form: text3 });
  } else if (msg instanceof UserClickButton) {
    return model.withFields({
      club_stored: model.club_form,
      number_of_shots_to_go: handle_shots_input(model.number_of_shots_form),
      view: new OutputView()
    });
  } else if (msg instanceof UserGoesToNextShot) {
    return handle_next_shot(model);
  } else {
    return model.withFields({ view: new InputView() });
  }
}
function welcome_view(_) {
  return div(
    toList([]),
    toList([
      text2(
      ...
      br(toList([])),
      button(
        toList([on_click(new UserGoesToInput())]),
        toList([text2("Go to input")])
      )
    ])
  );
}
function input_view(model) {
  return div(
    toList([]),
    toList([
      form(
        toList([on_submit(new UserClickButton())]),
        toList([
          text2("First club: "),
          input(
            toList([
              type_("text"),
              value(model.club_form.first),
              on_input(
                (var0) => {
                  return new UserChangeFirstClubName(var0);
                }
              )
            ])
          ),
          ...
          input(
            toList([
              type_("text"),
              value(model.club_form.second),
              on_input(
                (var0) => {
                  return new UserChangeSecondClubName(var0);
                }
              )
            ])
          ),
          ...
          input(
            toList([
              type_("text"),
              value(model.club_form.third),
              on_input(
                (var0) => {
                  return new UserChangeThirdClubName(var0);
                }
              )
            ])
          ),
          br(toList([])),
          text2("Number of balls to play: "),
          input(
            toList([
              type_("value"),
              value(model.number_of_shots_form),
              on_input(
                (var0) => {
                  return new UserChangeNumberOfShots(var0);
                }
              )
            ])
          ),
...
function select_random_club(model) {
  let $ = random_sample(int3(1, 3));
  if ($ === 1) {
    return model.club_stored.first;
  } else if ($ === 2) {
    return model.club_stored.second;
  } else {
    return model.club_stored.third;
  }
}
function select_random_cue(cue) {
  let idx = random_sample(int3(0, length(cue.values) - 1));
  let $ = (() => {
    let _pipe = cue.values;
...
var shot_cues = toList([
  new Cue(
    "Distance",
    toList([
      "10m shorter than normal",
      "5m shorter than normal",
      " your normal distance",
      "5m longer than normal",
      "as far as you can"
    ])
  ),
  new Cue(
    "Curve",
    toList(["draw", "slight draw", "straight", "slight fade", "fade"])
  ),
  new Cue(
    "Height",
    toList(["stinger", "lower", "normal height", "heigher", "flop-like"])
  ),
  new Cue("direction", toList(["pull", "straight", "push"]))
]);
function make_cues_text() {
  let _pipe = shot_cues;
  let _pipe$1 = map(
    _pipe,
    (cue) => {
      let value3 = select_random_cue(cue);
      return text2(cue.name + ": " + value3);
    }
  );
  return intersperse(_pipe$1, br(toList([])));
}
function output_view(model) {
  return div(
    toList([]),
    toList([
      text2(
        "There are " + to_string2(model.number_of_shots_to_go) + " out of " + model.number_of_shots_form + " shots to go"
      ),
      br(toList([])),
      text2("Hit your: " + select_random_club(model)),
      div(toList([]), make_cues_text()),
      br(toList([])),
      button(
        toList([on_click(new UserGoesToNextShot())]),
        toList([text2("Return to input")])
      )
    ])
  );
}
function view(model) {
  let $ = model.view;
  if ($ instanceof WelcomeView) {
    return welcome_view(model);
  } else if ($ instanceof InputView) {
    return input_view(model);
  } else {
    return output_view(model);
  }
}
function main() {
